...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("log/pose_estimation.log"), 
...
def process_video(input_path: str, output_path: str) -> None:
     ...
     while True:
         ...
         estimated_frame = pose_estimation(image = frame, matrix_coefficients_path="input\calibration_matrix.npy", distortion_coefficients_path="input\distortion_coefficients.npy")
         frame_count += 1
...
def pose_estimation(image: np.ndarray,matrix_coefficients_path: (str), distortion_coefficients_path: (str),marker_length: float = 0.02, aruco_dict_type: int = cv2.aruco.DICT_6X6_250) -> np.ndarray:
    """
